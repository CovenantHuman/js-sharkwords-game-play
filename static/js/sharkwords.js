...
const generateLetterButtons = () => {
  ...
  for (const char of ALPHABET) {
    letterButtonContainer.insertAdjacentHTML('beforeend', `<button id="${char}">${char}</button>`);
  }
};
...
const handleCorrectGuess = (letter) => {
  const correctGuesses = document.querySelectorAll(`.${letter}`);
  for (const correctGuess of correctGuesses) {
    correctGuess.innerHTML=`${letter}`;
  }
  let hasWon = true;
  for (let i=0; i<document.querySelectorAll('#word-container div').length; i+= 1) { 
    if (document.querySelectorAll('#word-container div')[i].innerHTML === "") {
      hasWon = false;
    }
  } 
  if (hasWon === true) {
...
const handleWrongGuess = () => {
  ...
  if (numWrong === 5) {
    for (const letter of ALPHABET) {
      const char = document.querySelector(`#${letter}`);
      disableLetterButton(char);
    }
